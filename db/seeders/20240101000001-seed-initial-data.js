/**
 * @fileoverview Sequelize seeder to populate the database with initial PetClinic data.
 * This seeder script inserts sample data into the `vets`, `specialties`, `vet_specialties`,
 * `types`, `owners`, `pets`, and `visits` tables. It closely mirrors the
 * `postgres/data.sql` found in the original Java project.
 *
 * Seeders are used for populating initial data, typically after migrations.
 */
...
module.exports = {
  /**
   * @method up
   * @description Inserts initial data into the database.
   * Data is inserted using `bulkInsert` to efficiently add multiple records.
  ...
  async up(queryInterface, Sequelize) {
    ...
    ], { ignoreDuplicates: true }); // ignoreDuplicates to handle idempotent insertions
  ...
  /**
   * @method down
   * @description Reverts the data insertions by deleting all records from the tables.
   * Tables are cleared in reverse order to respect foreign key constraints.
   * @param {import('sequelize').QueryInterface} queryInterface - Sequelize QueryInterface instance.
   * @param {import('sequelize').Sequelize} Sequelize - Sequelize library instance.
   * @returns {Promise<void>} A promise that resolves when the seeding rollback is complete.
   */
