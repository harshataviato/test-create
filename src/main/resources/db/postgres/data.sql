// This is a SQL script for PostgreSQL, providing idempotent initial data for the PetClinic application.
// It uses `WHERE NOT EXISTS` clauses to ensure that rows are only inserted if they don't already exist
// (e.g., based on `id` for vets/owners/pets, or `name` for specialties/types).
// It also uses `ON CONFLICT DO NOTHING` for `vet_specialties` to handle potential duplicate inserts
// based on the unique constraint on `(vet_id, specialty_id)`.
//
// This is not Java code and cannot be converted to TypeScript.
// In a Node.js/TypeScript project, database seeding can be done using:
// - **TypeORM/Prisma Migrations & Seeds**: ORMs provide CLI tools to generate migrations and seed data
//   (e.g., `typeorm migration:run`, `typeorm seed:run`). The seed data would be written in TypeScript
//   using the ORM's entity managers.
// - **Custom SQL scripts**: Executing these `.sql` files directly via a database client library
//   (e.g., `pg` for PostgreSQL, `mysql2` for MySQL) in a Node.js script (e.g., `src/db/seed.ts`).
// - **Faker.js**: For generating large amounts of fake data programmatically.
//
// The idempotent nature (checking `EXISTS` or `ON CONFLICT`) is good practice and would be maintained
// whether you use raw SQL or ORM seeding logic.
